...
NSString * AVF_Week2Project$ModuleRequireFormat = @"(function(exports){"
...
@implementation AVF_Week2ProjectObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_avf_week2project);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create AVF_Week2Project global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "AVF_Week2Project" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_avf_week2project = [[AVF_Week2ProjectObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_avf_week2project];
	
	NSString *_avf_week2projectNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _avf_week2projectNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_avf_week2project valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:AVF_Week2Project$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.avf_week2project.kroll" 
