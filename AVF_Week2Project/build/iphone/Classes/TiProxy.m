...
//Rare exceptions to indicate a bug in the _avf_week2project code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_avf_week2project = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_avf_week2project]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.avf_week2project.module"
