...
//Rare exceptions to indicate a bug in the _closegas code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_closegas = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_closegas]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.closegas.module"
