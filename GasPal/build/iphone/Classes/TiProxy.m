...
//Rare exceptions to indicate a bug in the _crimefind code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_crimefind = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_crimefind]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.crimefind.module"
